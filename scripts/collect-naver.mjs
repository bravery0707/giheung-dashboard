...
import { readNaverDOM, naverListReady, screenTotals, expectedAds, envelopeFromScreens } from './naver-dom.mjs';

export const NAVER_URL = 'https://fin.land.naver.com/complexes/109996';
export const LIMITS = Object.freeze({ requests: 600, staticRequests: 500, dataRequests: 100, actions: 45, durationMs: 240000, pauseMs: 2000 });
...
export class BrowserBudget {
  constructor(limits = LIMITS) { this.limits = limits; this.requests = 0; this.staticRequests = 0; this.dataRequests = 0; this.actions = 0; this.types = {}; this.error = null; }
  stop(code, message) { if (!this.error || code === 'ACCESS_DENIED') this.error = fault(code, message); return this.error; }
  check() { if (this.error) throw this.error; }
  request(type) {
    if (this.error) return false;
    this.types[type] = (this.types[type] || 0) + 1;
    this.requests++;
    if (['script', 'stylesheet', 'manifest'].includes(type)) this.staticRequests++;
    else this.dataRequests++; // Unknown request types consume the stricter data budget.
    if (this.requests > this.limits.requests) { this.limitReached = 'total-requests'; this.stop('BUDGET', '전체 브라우저 요청 상한에 도달했습니다.'); return false; }
    if (this.staticRequests > (this.limits.staticRequests ?? this.limits.requests)) { this.limitReached = 'static-requests'; this.stop('BUDGET', '화면 정적 파일 요청 상한에 도달했습니다.'); return false; }
    if (this.dataRequests > this.limits.dataRequests) { this.limitReached = 'data-requests'; this.stop('BUDGET', '문서·데이터 요청 상한에 도달했습니다.'); return false; }
    return true;
  }
...
export async function captureNaver({ chromium, profile, report = () => {} }) {
  ...
  try {
    ...
    await context.route('**/*', async route => {
      ...
      if (!['image', 'media', 'font'].includes(req.resourceType()) && !budget.error) {
        const host = new URL(req.url()).hostname;
        requestHosts[host] = (requestHosts[host] || 0) + 1;
      }
    ...
    context.on('response', response => {
      ...
      if ((url.hostname === 'naver.com' || url.hostname.endsWith('.naver.com') || url.hostname === 'pstatic.net' || url.hostname.endsWith('.pstatic.net')) && [401, 403, 429].includes(response.status())) {
    ...
    await page.waitForFunction(naverListReady, null, { timeout: 20000 }).catch(async error => { await checkScreen(); throw error; });
    let before;
    ...
    return { envelope, audit: { startedAt, capturedAt, requests: budget.requests, staticRequests: budget.staticRequests, dataRequests: budget.dataRequests, actions: budget.actions,
    ...
    if (page && !page.isClosed()) {
      try {
        screen = await page.evaluate(() => {
          ...
          return { rootPresent: !!root, titles: Array.from(root?.querySelectorAll('h3') || []).map(h => h.textContent.trim()).slice(0, 10),
            selectedTab: root?.querySelector('[role="tab"][aria-selected="true"]')?.textContent.trim() || '',
            filters: Array.from(root?.querySelectorAll('button') || []).map(b => b.textContent.trim()).filter(t => /^전체(거래유형|면적|동)$/.test(t)),
            cards: root?.querySelectorAll('[data-sentry-component="ArticleCard"]').length || 0 };
        });
      } catch { /* The browser may have closed after an access denial. */ }
    }
    failure.audit = { startedAt, finishedAt: new Date().toISOString(), status: 'failed', reason: failure.code || 'CHECK_FAILED',
      requests: budget.requests, staticRequests: budget.staticRequests, dataRequests: budget.dataRequests, actions: budget.actions, requestTypes: budget.types, requestHosts,
      phase, screen, limitReached: budget.limitReached || null, limits: LIMITS };
    throw failure;
  } finally {
    clearTimeout(timer);
...
export async function collectNaver({ config, directory = PRIVATE_DATA, now = new Date(), capture = captureNaver, chromium, report = () => {}, manualRetry = false, retryReason = '' }) {
  ...
  return withDataLock(directory, async dir => {
    ...
    const repeated = state.lastAttemptDate && state.lastAttemptDate >= today;
    if (repeated) {
      const last = await readOptionalJson(join(dir, 'naver-last-attempt.json'), null);
      // Explicit operator-only extra diagnostic run: once, after a budget failure.
      // The scheduler never supplies this flag. Denial and trial limits cannot be overridden.
      if (!manualRetry || state.lastAttemptDate !== today || state.lastManualRetryDate === today || !retryReason.trim() || last?.reason !== 'BUDGET' || kstDate(last.startedAt) !== today) throw fault('DAILY_LIMIT', '오늘 수집은 이미 시도했습니다. 자동 재시도하지 않습니다.');
    } else if (manualRetry) throw new Error('추가 시험은 오늘 요청 상한으로 실패한 실행에만 사용할 수 있습니다.');
    const reserved = { ...state, lastAttemptDate: today, attemptedAt: now.toISOString(), mode: 'naver-browser',
      ...(repeated ? { lastManualRetryDate: today, manualRetryReason: retryReason.trim().slice(0, 300) } : {}) };
...
async function main() {
  ...
  const result = await collectNaver({ config, chromium, directory: resolve(process.env.LISTING_DATA_DIR || PRIVATE_DATA), report: console.log,
    manualRetry: process.argv.includes('--manual-budget-retry'), retryReason: reasonIndex < 0 ? '' : (process.argv[reasonIndex + 1] || '') });
